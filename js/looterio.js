...
(function () {
  ...
  var onScreen = true, odd = false, id = null, idW = 0, idH = 0;
  if ('IntersectionObserver' in window) new IntersectionObserver(function (es) { onScreen = es[0].isIntersecting; }).observe(wrap);
  function frame() {
    if (!onScreen) { setTimeout(frame, 300); return; }
    odd = !odd; if (odd) { requestAnimationFrame(frame); return; }
    t += 0.036;
    var wa = 1.4, pw = oc.width, ph = oc.height;
    if (!id || idW !== pw || idH !== ph) { id = ox.getImageData(0, 0, pw, ph).data; idW = pw; idH = ph; }
    var str = '';
