...
const PRODUCTS = [
  ...
  { id: 'snowball', cat: ['software', 'custom', 'capital'], tile: 'snowball', img: 'assets/snowball-tile.png', title: 'Snowball. we scale companies', priceLabel: '฿?', note: 'scale', link: 'https://looterstudio.xyz/snowball/' },
  { id: 'louvre',   cat: ['custom', 'capital'], tile: 'louvre', img: 'assets/luv-tile.png', title: 'LUV Agency. OF management', priceLabel: '฿?', note: 'management', link: 'https://looterstudio.xyz/luv/' },
  ...
  { id: 'cult',   cat: ['apparel', 'custom'],  tile: 'cult',  img: 'assets/illuminati.jpg', title: 'ILLUMINATI MEMBERSHIP. no refunds', price: 4.20, note: 'soon' },
  { id: 'cherry', cat: ['capital', 'custom', 'art'], tile: 'cherry', img: 'assets/cherry-cupid.jpg?v=3', title: 'CHERRY BOMB', priceLabel: '฿?', note: 'soon' },
...
/* Keyed 360: the spinning logo is a pre-keyed animated WebP with alpha (20 fps,
   made from loot-360.mp4 by tools/make-360.sh). One <img> per slot: the browser
   composites it on the GPU, no per-frame JavaScript, no video decoder, no lag.
   The <video> tags in the HTML are only the fallback for browsers without WebP. */
function keyVideos() {
  const src = (window.LOOT_ASSETS || '') + 'assets/loot-360-alpha.webp';
  const toImage = (v) => {
    const img = document.createElement('img');
    img.src = src; img.alt = ''; img.decoding = 'async'; img.loading = 'eager';
    img.className = v.className.replace('k360', 'k360-out');
    v.replaceWith(img);
    return img;
  };
  document.querySelectorAll('video.k360').forEach(toImage);
  window.keyed360 = { attach: toImage };
}
